...
- 跨标签页通信
- 浏览器架构
- 浏览器下[事件循环](https://segmentfault.com/a/1190000016278115)（`Event Loop`）：同步执行代码，依次将代码放入到调用栈中执行。遇到宏任务，将其回调函数押入宏任务队列，遇到微任务，将其押入微任务队列。然后依次取出微任务队列中的任务执行，微任务执行完后，再依次取出宏任务队列中的任务执行。
